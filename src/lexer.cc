  ...
  while (peek().has_value()) {
    if (std::isalpha(peek().value()) || peek().value() == '_' ||
      ...
      if (buf == "var") {
        ...
        tokens.push_back(
            {.type = TokenType::plus_eq, .value = "+=", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "subing") { // substracting
        tokens.push_back(
            {.type = TokenType::minus_eq, .value = "-=", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "muling") { // multiplicating
        tokens.push_back(
            {.type = TokenType::star_eq, .value = "*=", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "diving") { // dividing
        tokens.push_back(
            {.type = TokenType::slash_eq, .value = "/=", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "inc") {
        tokens.push_back(
            {.type = TokenType::plusplus, .value = "++", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "dec") {
        tokens.push_back(
            {.type = TokenType::minusminus, .value = "--", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "if") {
        ...
        tokens.push_back(
            {.type = TokenType::_else, .value = "else", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "elif") {
        tokens.push_back(
            {.type = TokenType::_elif, .value = "elif", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "and") {
        ...
        tokens.push_back(
            {.type = TokenType::eq_eq, .value = "==", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "neq") {
        tokens.push_back(
            {.type = TokenType::bang_eq, .value = "!=", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "gt") {
        ...
        tokens.push_back(
            {.type = TokenType::_nullptr, .value = "nil", .line = local_lines});
        buf.clear();
        continue;
        ...
        tokens.push_back(
            {.type = TokenType::_undef, .value = "$undef", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$isdef") {
        tokens.push_back(
            {.type = TokenType::_is_def, .value = "$isdef", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$isndef") {
        tokens.push_back(
            {.type = TokenType::_is_ndef, .value = "$isndef", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$if") {
        tokens.push_back(
            {.type = TokenType::_pre_if, .value = "$if", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$elif") {
        tokens.push_back(
            {.type = TokenType::_pre_elif, .value = "$elif", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$else") {
        tokens.push_back(
            {.type = TokenType::_pre_else, .value = "$else", .line = local_lines});
        buf.clear();
        continue;
      } else if (buf == "$endif") {
        tokens.push_back(
        ...
        tokens.push_back({.type = TokenType::_pre_warning,
                          .value = "$warning",
                          .line = local_lines});
        buf.clear();
        continue;
        ...
        tokens.push_back(
            {.type = TokenType::_size_of, .value = "$sizeof", .line = local_lines});
        buf.clear();
        continue;
      } else {
      ...
      tokens.push_back({.type = is_float ? TokenType::float_lit : TokenType::int_lit,
                        .value = buf,
                        .line = local_lines});
      buf.clear();
      ++tokens_in_current_line;
      ...
      tokens.push_back(
          {.type = TokenType::char_lit, .value = buf, .line = local_lines});
      buf.clear();
      ++tokens_in_current_line;
    } else if (peek().value() == '(') {
      consume();
      tokens.push_back(
          {.type = TokenType::open_paren, .value = "(", .line = local_lines});
      ++tokens_in_current_line;
      continue;
    } else if (peek().value() == ')') {
      consume();
      tokens.push_back(
          {.type = TokenType::close_paren, .value = ")", .line = local_lines});
      ++tokens_in_current_line;
      continue;
    } else if (peek().value() == ';') {
      consume();
      ...
      consume();
      while (peek().has_value()) {
        if (peek().has_value() && peek().value() == '*' && peek(1).has_value() &&
            peek(1).value() == '#')
          break;
        consume(); // ignore all inside of comment
      }
      if (peek().has_value() && peek().value() == '*' && peek(1).has_value() &&
          peek(1).value() == '#') {
        consume();
