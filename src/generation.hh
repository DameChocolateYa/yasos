...
void check_func_args(
    const std::vector<NodeExprPtr> &args,
    const std::unordered_multimap<ArgType, ArgRequired> &required_args);

extern std::unordered_map<std::string, Type> known_function_types;

extern int main_func_declared;

enum class PrintType : int { Int = 0, Float = 1, Str = 2, CR = 3 };
...
extern std::unordered_map<std::string, std::pair<Type, std::vector<Type>>>
    declared_funcs;

extern std::set<std::string> m_preprocessor_bool;
extern std::unordered_map<std::string, std::vector<Token>> m_preprocessor;
...
public:
  ...
  std::map<std::string, std::map<std::string, std::pair<int, llvm::Type *>>>
  ...
  inline void write(const std::string &output, int newline = true) {
    ...
    if (current_mode == Mode::Function)
      function_buffer << output << (newline ? "\n" : "");
    else
      main_buffer << output << (newline ? "\n" : "");
  }
