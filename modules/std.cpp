...
extern "C" void printbp(const char* format, ...) {
    if (!format) {
...
extern "C" int isint(const char* s1) {
...
extern "C" int isfloat(const char* s1) {
    int decimal = false;
    for (int i = 0; s1[i] != '\0'; ++i) {
...
extern "C" int isnum(const char* s1) {
    int decimal = false;
    for (int i = 0; s1[i] != '\0'; ++i) {
        if (s1[i] != '-' && s1[i] != '.' && (s1[i] < '0' || s1[i] > '9')) {
            return 0;
...
extern "C" int sysexc(const char* command) {
...
extern "C" void termc(const int color) {
    if (color >= 0 && color <= 7) {
...
extern "C" uint32_t get_secure_random_uint32() {
    uint32_t num;
...
extern "C" int randi(int min, int max) {
    uint32_t r = get_secure_random_uint32();
    return (r % (max - min + 1)) + min;
}

extern "C" double randf(double min, double max, int decimals) {
	uint32_t r = get_secure_random_uint32();
...
extern "C" int randb(double probability = 0.5) {
	...
	double normalized = (double)r / (double)UINT32_MAX;
	return normalized < probability ? 1 : 0;
}

extern "C" void nap(const unsigned int seconds) {
	sleep(seconds);
}
