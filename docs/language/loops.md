...
We use it to repeat a code without an explicit condition
`
loop {
    code...
}
...
We use the instruction stop when we do not want to keep itinerating in the loop.
We use the instruction continue when we want no go to the init of the loop's code
